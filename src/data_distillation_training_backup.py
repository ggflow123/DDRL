...
def train_student_data_distillation_episodic(student: Student, teacher: Teacher, env: gymnasium.Env or gym, 
                           loss_function: LossFunction, num_episodes=100, optimizer=None, 
                           online_network="student", max_steps=None, student_train_epochs=10, student_update_freq=16, rollout_buffer=None, synthetic_buffer=None, manual_init=True, rng=np.random.default_rng(), batch_size=1, device='cpu'):
    '''
            ...
            if not synthetic_buffer.is_synthetic_initialized:
                ...
                if synthetic_buffer.is_init_optimizer: # Make this is_initialized() by synthetic buffer.
                    if not manual_init:                    
                        ...
                        states_syn = torch.empty((15*4, *env.observation_space.shape), dtype=torch.float32, device=device)
                        teacher_knowledge_syn = torch.empty((15*4, *(env.action_space.n, )), dtype=torch.float32, device=device)
                        num = 4 # TODO: make it configured by hydra.
                        for action in range(15): # actions
                            index = action
                    ...
                    synthetic_data = torch.nn.ParameterList([state_param, teacher_knowledge_param])
                    #synthetic_teacher = [teacher_knowledge_param, ]
                    optimizer_states_syn = torch.optim.Adam(synthetic_data, lr=0.01)
                ...
                #print(rollout_buffer.buffer)
                real_batch = rollout_buffer.sample(batch_size)
                #syn_outer_batch = synthetic_buffer.sample(batch_size)
                if batch_size == 1:
                # calculate loss. TODO: add support for multiple batch sizes
                    real_student_knowledge = student.get_knowledge(real_batch[0][0])

                    real_loss = loss_function.loss(real_student_knowledge, real_batch[0][1])
                    syn_student_knowledge = student.get_knowledge(synthetic_data[0])
                    syn_loss = loss_function.loss(syn_student_knowledge, synthetic_data[1])
                else:
                ...
                synthetic_buffer.synthetic_knowledge_buffer = synthetic_data[1].clone().detach().to(device)
                ...
                syn_batch = [copy.deepcopy(synthetic_data[0].clone().detach().to(device)), copy.deepcopy(synthetic_data[1].clone().detach().to(device))]
