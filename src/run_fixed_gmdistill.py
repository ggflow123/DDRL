...
def main(cfg: DictConfig):
    ...
    num_of_data = cfg.data_size
    state_shape = env.observation_space.shape
    action_size = env.action_space.n

    states_path = os.path.join(data_path, f"{env_name}-{num_of_data}-trained-distilled-states.pkl")
    teacher_knowledge_path = os.path.join(data_path, f"{env_name}-{num_of_data}-trained-distilled-actions.pkl")
    ...
    with open(states_path, 'rb') as f:
        states_offline = pickle.load(f)

    with open(teacher_knowledge_path, 'rb') as f:
        actions_offline = pickle.load(f) 
    # configure teacher
