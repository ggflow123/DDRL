...
from behavioral_cloning import train_student_behavioral_cloning_episodic_offline
...
def main(cfg: DictConfig):
    ...
    save_names.append(f"bc-{percentage}")
    ...
    train_student_behavioral_cloning_episodic_offline(student=student, teacher=teacher, env=env, loss_function=loss_function, data_path=data_path, percentage=percentage, optimizer=optimizer, rollout_buffer=rollout_buffer, synthetic_buffer=trainable_buffer, student_train_epochs=cfg.student_train_epochs, student_update_freq=cfg.student_update_freq, device=device, **distill_kwargs)
    save_names.append(cfg.distill.save_name)

    student_rewards = evaluator.evaluate(student, env, num_episodes=50)
